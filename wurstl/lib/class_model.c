/*
 * $Id: class_model.c,v 1.13 2014/06/25 08:53:42 torda Exp $
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
get_clssfcn(const char *influence_report_filename, const float abs_error)
{
    ...
    float dummy_f = 0.0;
    char dummy_c;
    char *dummy_s = E_MALLOC(5 * sizeof(char));
    char *dummy_str = E_CALLOC(80, sizeof(char));
    float mean = 0, dev = 0;
    size_t corr_idx = 0;
	...
	memset (dummy_s, 0, DUMMY_S_SIZE);
	memset (dummy_str, 0, DUMMY_S_SIZE);
    if ((influence_report_file = mfopen(influence_report_filename, "rb",
                                        this_sub)) == NULL) {
        return NULL;
    }
    ...
    while (fscanf(influence_report_file,
                  ...
                  "num description %s",
                  dummy_str) != 1 && strcmp(dummy_str, "I-*k")) {
        if (feof(influence_report_file)) {
            err_printf(this_sub,"unexpected EOF reached in file %s!\n",
                       influence_report_filename);
            free_if_not_null(cmodel->class_weight);
   ...
   if (fgetpos(influence_report_file, &pos) != 0){
    ...
    for (i = 0; i < cmodel->n_class; i++) {
        ...
        while (i_){
            ...
            else if (dummy_s[0] =='a'){  /* Add an action if r has */
                int r;                   /* wrong value. Broken code */
                r = fscanf (influence_report_file, "%*f %*s %*e %*e %*e");
                for (k = 1; k < MIN_AA; k++) {
        ...
        if (fscanf (influence_report_file, "%s", dummy_str) != 1) {
            scanf_error (cmodel, i, attr_idx, influence_report_filename);
            return NULL;
        }
        ...
        while (fscanf(influence_report_file, "%s ", dummy_str) == 1
            ...
            for (k = 0; k < cmodel->dim; k++) {
                if (use_idx[k] == 0)
                    continue;
                if (fscanf(influence_report_file, "%u", (unsigned int*)&dummy_i) != 1) {
                    scanf_error (cmodel, i, attr_idx, influence_report_filename);
                    return NULL;
                }
                ...
                for (l = 0; l < cmodel->dim; l++) {
                    if (use_idx[l] == 0)
                        continue;
                    if (fscanf(influence_report_file, "%lf",
                               &cmodel->cov_matrix[i][(k*cmodel->dim)+l]) != 1) {
                        scanf_error (cmodel, i, attr_idx, influence_report_filename);
                        return NULL;
                    }
                        ...
                        const struct clssfcn *cmodel)
{
    ...
    for (i = 0; i < cmodel->n_class; i++) {   /*for every class...*/
        ...
        case SINGLE_NORMAL:
            for (j = 0; j < cmodel->dim; j++) {
                double dist = (test_vec[j] == BAD_ANGLE ? 0 
                               : test_vec[j] - cmodel->param[i][j][0]);
                if (dist > M_PI) {
                    dist -= 2 * M_PI;
                } else if (dist < -M_PI) {
                    dist += 2 * M_PI;
                ...
                mship[i] *=
                    (gsl_sf_erf_Q((dist / cmodel->param[i][j][1])
                                  - fabs(cmodel->abs_error)) -
                     gsl_sf_erf_Q((dist / cmodel->param[i][j][1])
                                  + fabs(cmodel->abs_error)));
            }
            break;
        case MULTI_NORMAL: {
            ...
            exponent *= -0.5;
            mship[i] *= pow(2 * cmodel->abs_error, cmodel->dim) * /* not needed if normalised */
                exp(exponent) / sqrt(pow(2 * M_PI, cmodel->dim) * fabs(C_det));
