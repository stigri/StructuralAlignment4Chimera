 ...
 * $Id: score_mat.c,v 1.3 2011/09/07 14:36:04 margraf Exp $
 */
 ...
 */
int
score_mat_write_gnuplot( const struct score_mat *smat, const char *fname,
...
int score_metric(const struct score_mat *smat, struct pair_set *pairs)
{
    int distance = 0;
    int i = 0;
    for (i=0; i < pairs->n; i++){
        if ((pairs->indices[i][0] != -1) && (pairs->indices[i][1] != -1)){
/*
 *         mprintf("matrix %d, %d is %f", pairs->indices[i][0], pairs->indices[i][0], smat->mat[pairs->indices[i][0]][pairs->indices[i][0]] );
 */          
          if ( smat->mat[pairs->indices[i][0]][pairs->indices[i][0]] < 3.0){
                distance++;
            }
        }
    }
