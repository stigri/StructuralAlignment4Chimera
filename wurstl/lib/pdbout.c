 ...
 * $Id: pdbout.c,v 1.2 2009/01/20 17:30:03 schenk Exp $
 */
...
/* ---------------- coord_pdbable -------------------------------
 * check all atom coordinates if they are printable in pdb format
 */
static enum yes_no
coord_pdbable(const struct coord * c)
{
    size_t i;
    for (i = 0; i < coord_size(c); ++i) {
        if (c->rp_ca[i].x >= 10000 || c->rp_ca[i].x <= -1000
            || c->rp_ca[i].y >= 10000 || c->rp_ca[i].y <= -1000
            || c->rp_ca[i].z >= 10000 || c->rp_ca[i].z <= -1000
            || c->rp_cb[i].x >= 10000 || c->rp_cb[i].x <= -1000
            || c->rp_cb[i].y >= 10000 || c->rp_cb[i].y <= -1000
            || c->rp_cb[i].z >= 10000 || c->rp_cb[i].z <= -1000
            || c->rp_c[i].x >= 10000 || c->rp_c[i].x <= -1000
            || c->rp_c[i].y >= 10000 || c->rp_c[i].y <= -1000
            || c->rp_c[i].z >= 10000 || c->rp_c[i].z <= -1000
            || c->rp_o[i].x >= 10000 || c->rp_o[i].x <= -1000
            || c->rp_o[i].y >= 10000 || c->rp_o[i].y <= -1000
            || c->rp_o[i].z >= 10000 || c->rp_o[i].z <= -1000
            || c->rp_n[i].x >= 10000 || c->rp_n[i].x <= -1000
            || c->rp_n[i].y >= 10000 || c->rp_n[i].y <= -1000
            || c->rp_n[i].z >= 10000 || c->rp_n[i].z <= -1000) {
            return NO;
        }
    }
    return YES;
...
coord_2_pdb (const char *fname, struct coord *c, struct seq *seq)
{
    ...
    if (! coord_pdbable(c)) {
        err_printf(this_sub, "coordinates not pdbable\n");
        return EXIT_FAILURE;
    }
...
{
    const char * this_sub = "coord_2_cptn";
...
coord_2_spdb ( const char *fname, struct coord *c,
               struct seq *seq, struct scor_set *scr)
{
    const char * this_sub = "coord_2_spdb";
    enum yes_no cptn_flag = NO;
    if (! coord_pdbable(c)) {
        err_printf(this_sub, "coordinates not pdbable\n");
        return EXIT_FAILURE;
