 ...
 * $Id: pdbin.c,v 1.5 2014/06/12 11:13:49 torda Exp $
 */
...
void
check_cmpnd ( struct coord *c)
{
    const char *this_sub = "check_cmpnd";
    if (c->compnd_len == 0) {
        err_printf(this_sub,"Putting Dummy compound!\n");
        c->compnd = E_MALLOC (6 * sizeof(char));
        strncpy (c->compnd,"dummy\0",6);
        c->compnd_len = 6;
    }
}
